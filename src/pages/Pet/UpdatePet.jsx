...
const UpdatePet = () => {
  ...
  useEffect(() => {
    setPetData({ ...pet, species_id: pet.species.id });
    setOldImage(pet.image);
  }, []);
  ...
  const handleUploadFile = async () => {
    ...
    let newUpload = {};
    if (hasimageFile) {
      const [deleteAvatarResult, deleteAvatarError] = await UploadService.deleteFile(oldImage);
  ...
  const onSubmit = async () => {
    ...
    const [result, error] = await PetService.update({
      ...petData,
      image,
    });
    if (error) {
